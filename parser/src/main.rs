...
fn classify(root: &Path) -> Result<(), Box<dyn Error>> {
    ...
    let new_folder = String::from("parsed_data_generalized/"); 
    ...
    //         let entry = entry?;
    for entry in 
        WalkDir::new(&root).into_iter().filter_entry(|e| e.file_name() != "auxiliary")
    {
        let entry = entry?;
        if !entry.file_type().is_dir() && entry.file_name() != ".gitattributes" {
            ...
            if let Ok(s) = fs::read_to_string(&path) {
                ...
                // begin
                new_file.push_str(&new_folder);
                new_file.push_str(&file_name);

                let (create_folder, _) = new_file.rsplit_once("/").unwrap();
                fs::create_dir_all(create_folder)?;

                let mut file = fs::File::create(new_file.clone())?;
                for token in tokenize(&s) {
                    // println!("{} {} {} {}", token, generalize(&token), &path.display(), &file_name);
                    // file.write(token.as_bytes())?;
                    file.write(generalize(&token).as_bytes())?;
                    file.write(b"\n")?;
                }
                new_file.clear();
                // end                 
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let root = Path::new("/home/huawei123/kwx1991442/code-classification/data/rust/tests/ui/");
